...
_(Заметка)    
Markdown позволяет также использовать неявно выраженный идентификатор (пустой). В этом случае `[][метка]` не указывается, вместо неё  используется текст гиперсылки, а вторая пара квадратных скобок остаётся пустой `[name-url][]`.    
